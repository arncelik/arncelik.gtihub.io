		...
		class Revolve_Welcome {
			...
			public function __construct() {
				...
				$this->tab_sections = array(
					...
					'more_wp_stuff' => __('More WordPress Stuff', 'revolve'),
				);
					...
					<div class="notice notice-success is-dismissible">
						<p>
							<?php
								...
								printf( wp_kses( 'Welcome! Thank you for choosing %1$s! Please make sure you visit our <a href="%2$s">Welcome page</a> to get started with Revolve.', array( 'a' => array( 'href' => array() ) ) ), esc_html($this->theme_name), esc_url(admin_url( 'themes.php?page=revolve-welcome' ))  );
							?>
						</p>
						<p><a class="button" href="<?php echo esc_url(admin_url( 'themes.php?page=revolve-welcome' )); ?>"><?php esc_html_e( 'Lets Get Started', 'revolve' ); ?></a></p>
					</div>
					<?php
				}
			}
			...
			public function revolve_welcome_screen() {
				...
				<div class="wrap about-wrap access-wrap">
					<h1><?php /* translators: %1$s : theme name, %2$s : theme version */ printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'revolve' ), esc_html($this->theme_name), esc_html($this->theme_version) ); ?></h1>
					<div class="about-text"><?php /* translators: %s : theme name */ printf( esc_html__( 'The %s is a Beautiful custom designed modern free WordPress theme with elegant design perfect for portfolio, personal blog, business, photography or any other use. It is feature-rich, multi-purpose and powerful WordPress theme with a beautiful and user-friendly design. It offers a complete customization capability and multiple options for building a website instantly.', 'revolve' ), esc_html($this->theme_name) ); ?></div>
				...
				<div class="nav-tab-wrapper clearfix">
					<?php foreach($tabs as $id => $label) : ?>
						...
						?>
						<a href="<?php echo esc_url(admin_url('themes.php?page=revolve-welcome&section='.$id)); ?>" class="<?php echo esc_attr($nav_class); ?>" >
			...
			public function revolve_plugin_installer_callback(){
				...
				if ( ! current_user_can('install_plugins') )
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'revolve' ) );
				...
				if (! wp_verify_nonce( $nonce, 'revolve_plugin_installer_nonce' ))
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'revolve') );
			...
			public function revolve_plugin_activation_callback(){
				...
				if ( ! current_user_can('install_plugins') )
					wp_die( esc_html__( 'Sorry, you are not allowed to activate plugins on this site.', 'revolve' ) );
				...
				if (! wp_verify_nonce( $nonce, 'revolve_plugin_activate_nonce' ))
					die( esc_html__( 'Error - unable to verify nonce, please try again.', 'revolve' ) );
